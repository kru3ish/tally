...
const total = entries.reduce((n, e) => n + e.criteria.length, 0);
...
const values: Record<string, string> = {
  ...
  CALIBRATION_CRITERIA: String(entries.reduce((n, e) => n + e.criteria.length, 0)),
  CALIBRATION_CRITERION_AGREEMENT: pct(r.criterion_agreement),
  ...
  CALIBRATION_VERDICT_ABSTAINED: String(r.verdict_abstained),
...
const files = [path.join(root, 'README.md'), path.join(root, 'site', 'index.html'), ...fs.readdirSync(path.join(root, 'docs', 'launch')).filter((f) => f.endsWith('.md')).map((f) => path.join(root, 'docs', 'launch', f))];
