...
- SessionEnd hooks share a 1.5 s budget (raised to a per-hook `timeout` if set, max 60 s); hooks over budget are cancelled. Tally's SessionEnd hook only appends one event and spawns a detached `tally finalize`, measured at ~45 ms.
