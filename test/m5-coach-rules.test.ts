...
describe('rules', () => {
  it('has all 11 rules plus the consent prompt', () => {
    expect(RULES.map((r) => r.id)).toEqual(['loop-detect', 'reread', 'context-pressure', 'claude-md', 'mcp-opportunity', 'dead-weight', 'mcp-errors', 'permission-friction', 'burn-rate', 'task-quality', 'history-lesson', 'verification-consent', 'task-confirm', 'security-watch', 'rate-limit']);
  });
